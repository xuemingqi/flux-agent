...
- 模型设置可配置上下文窗口与输出预算；接近容量时自动生成摘要，摘要与完整原始历史分别保存到 SQLite。
...
文件读取与写入限制 64 KB，写入使用原子替换；父目录必须存在，审批后原文或目标路径变化则拒绝写入。搜索使用字面字符串，跳过 `.git`、`node_modules` 和符号链接，最多扫描 500 项、返回 50 条结果。审批可在刷新后继续处理，断开浏览器不等于拒绝；停止运行会取消待审批请求。
...
模型设置中的上下文窗口默认 32768 tokens，请按服务商实际容量调整。最大输出默认 0，表示不发送 `max_tokens`，由服务商决定输出预算；可选填写显式预算，思考与正文共同占用该预算。界面不再限制最多只能填写 8192。旧配置保持原值，可在设置页改为 0；模型明确返回长度截断时会展示原因并保存终止标识。

每次请求预留输出及 10% 余量，按字符类别与消息开销近似估算 token（并非供应商精确 tokenizer，不把 UTF-8 字节数直接当 token）；未指定输出上限时的预留量仅用于输入估算，不会发送为模型输出限制。接近输入预算时自动将较早的消息压缩为摘要，保留目标、最新调整、关键事实、操作结果与待办；工具调用及对应结果整体处理，当前执行单元不拆开，当前任务和调整消息的原文始终保留。工具结果不再按 4000 字符切掉中间内容，历史膨胀交由摘要处理。过长历史按估算 token 分块归纳，直接收集摘要流，不为统计用量额外下载 tokenizer。页面显示已完成片段数，刷新后也能看到当前压缩进度。
